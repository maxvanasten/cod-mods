...
config()
{
	cfg = spawnstruct();
	// Main timer config
	cfg.main_timer_color = (0.7, 0.1, 0.1);
	// Color of the main timer
	cfg.timer_font = "Objective";
	cfg.timer_font_size = 1.5;
	// Split ui config
	cfg.text_color = (0.7, 0.7, 0.7);
	// Color of the regular text
	cfg.delta_time_color = "^5";
	// Color of the time since the last split
	cfg.total_time_color = "^3";
	// Color of the time since the beginning of the game   
...
initialize()
{
	...
	// The time since the last split was completed
	// Initialize split variables
	self.splits = get_splits();
	// get_splits() is a function provided by the split file
	self.current_split = 0;
	// The index of the current split
	self.run_complete = 0;
	// = 1 when all of the splits are complete

	self.split_complete = 0;
	// = 1 when the current split is complete
...
main_loop()
{
	...
	if (self.split_complete == 1) {
		self.split_complete = 0;
		self.splits[self.current_split]
		.time_string = "<" + config();
...
update_split_ui()
{
	...
	for (i = 0; i < self.splits.size; i = i + 1)
	{
		content = content + "[" + self.splits[i];
	}
...
get_time_as_string(type)
{
	...
	if (r_seconds > 9) {
		result = result + r_seconds + ".";
	} else {
		result = result + "0" + r_seconds + ".";
	}
